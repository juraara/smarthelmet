...
Rect CV::detectEyes(Mat &frame, CascadeClassifier &eyeCascade) {
    ...
    if (circles.size() > 0) {
        ...
        CV::eyeData.radius = (int)eyeball[2]; // get iris radius
    ...
    // printf("y-coor (line) = %.0f ", getEyeball(eye, circles)[1]);
    // printf("x-coor = %d y-coor = %d\n", getLeftmostEye(eyes).x, getLeftmostEye(eyes).y);
...
void CV::detectBlink(Mat &frame) {
    ...
    Mat thresh;
    threshold(blur, thresh, 15, 255, THRESH_BINARY_INV); // convert to binary image
    ...
    //int upper_h = (int)((double)crop.rows * 0.50) + (int)((double)CV::CV::eyeData.radius * 2 * 0.3); // upper half and additional 3% of iris rad from the center should approximately include 80% of eyes.
    ...
    printf("Counter = %d counter limit = %d\n", CV::frameData.counter, CV::frameData.numOfFrames);
